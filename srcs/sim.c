...
/*   Updated: 2026/07/07 14:50:21 by rsao-pay         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (!request_dongle(coder->l_dongle, coder->sim))
		return ;
	write_status(TAKE_DONGLE, coder);
	if (!request_dongle(coder->r_dongle, coder->sim))
	{
		release_dongle(coder->l_dongle, coder->sim);
		return ;
	}
	write_status(TAKE_DONGLE, coder);
	...
	release_dongle(coder->l_dongle, coder->sim);
	release_dongle(coder->r_dongle, coder->sim);
}
